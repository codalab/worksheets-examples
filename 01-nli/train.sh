...
CODALAB_ARGS="$CODALAB_ARGS --request-time 10d"  # Kill job after this many days (default: 1 day)
...
CMD="$CMD --outputdir ."  # Runs can feel free to output to the current directory, which is in the bundle
CMD="$CMD --word_emb_path word-vectors.txt"
CMD="$CMD --train_frac 0.1"  # Make it train fast
CMD="$CMD --n_epochs 3"
if [ -n "$1" ]; then
  CMD="$CMD $@"  # Pass the command-line arguments through
fi
